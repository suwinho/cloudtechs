...
docker rm -f frontend api-a api-b 2>/dev/null || true
...
docker run -d --name api-a --network $NET -e INSTANCE_ID="Serwer-A" $BACK_IMG:latest
docker run -d --name api-b --network $NET -e INSTANCE_ID="Serwer-B" $BACK_IMG:latest
